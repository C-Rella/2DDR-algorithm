...
algo_2DDR <- function(score, # Estimated score (first dimension)
                      xi, # Exogenous variable (second dimension)
                      y, # Response variable
                      cost_matrix, # Cost matrix specifying costs (Two columns: \hat{Y} = 0 | \hat{Y} = 1)
                      k = 10, # k parameter
                      pp_max = 1, # Limit of observations labeled as case
                      represent = F, # Algorithm search representation
                      regular = F # Regular search (with quantile grid by default)
                      ){ 
